...
__name(buildOpenAIChatCompletionsUrl, "buildOpenAIChatCompletionsUrl");
...
// src/dataSources/comsoc.js
var ComsocDataSource = {
  fetch: /* @__PURE__ */ __name(async (env, foloCookie) => {
    const feedId = env.COMSOC_FEED_ID;
    const fetchPages = parseInt(env.COMSOC_FETCH_PAGES || "3", 10);
    const allComsocItems = [];
    const filterDays = parseInt(env.FOLO_FILTER_DAYS || "3", 10);
    if (!feedId) {
      console.error("COMSOC_FEED_ID is not set in environment variables.");
      return {
        version: "https://jsonfeed.org/version/1.1",
        title: "Comsoc.AI Daily Feeds",
        home_page_url: "https://techblog.comsoc.org/",
        description: "Aggregated Comsoc.AI Daily feeds",
        language: "en-US",
        items: []
      };
    }
    ...
    for (let i = 0; i < fetchPages; i++) {
      ...
      try {
        console.log(`Fetching Comsoc.AI data, page ${i + 1}...`);
        const response = await fetch(env.FOLO_DATA_API, {
          method: "POST",
          headers,
          body: JSON.stringify(body)
        ...
        if (!response.ok) {
          console.error(`Failed to fetch Comsoc.AI data, page ${i + 1}: ${response.statusText}`);
          break;
        }
        const data = await response.json();
        if (data && data.data && data.data.length > 0) {
          const filteredItems = data.data.filter((entry) => isDateWithinLastDays(entry.entries.publishedAt, filterDays));
          allComsocItems.push(...filteredItems.map((entry) => ({
            id: entry.entries.id,
            url: entry.entries.url,
            title: entry.entries.title,
            content_html: entry.entries.content,
          ...
          console.log(`No more data for Comsoc.AI, page ${i + 1}.`);
          break;
        }
      } catch (error) {
        console.error(`Error fetching Comsoc.AI data, page ${i + 1}:`, error);
        break;
      }
      await sleep(Math.random() * 5e3);
    }
...
var comsoc_default = ComsocDataSource;

// src/dataSources/lightreading.js
var LightreadingDataSource = {
  fetch: /* @__PURE__ */ __name(async (env, foloCookie) => {
...
var lightreading_default = LightreadingDataSource;

// src/dataSources/aliyun.js
var AliyunDataSource = {
  fetch: /* @__PURE__ */ __name(async (env, foloCookie) => {
    const feedId = env.ALIYUN_FEED_ID;
    const fetchPages = parseInt(env.ALIYUN_FETCH_PAGES || "3", 10);
    const allAliyunItems = [];
    const filterDays = parseInt(env.FOLO_FILTER_DAYS || "3", 10);
    if (!feedId) {
      console.error("ALIYUN_FEED_ID is not set in environment variables.");
      return {
        version: "https://jsonfeed.org/version/1.1",
        title: "Aliyun.AI Daily Feeds",
        home_page_url: "https://www.aliyun.com/",
        description: "Aggregated Aliyun.AI Daily feeds",
        language: "zh-cn",
        items: []
      };
    }
    ...
    for (let i = 0; i < fetchPages; i++) {
      ...
      try {
        console.log(`Fetching Aliyun.AI data, page ${i + 1}...`);
        const response = await fetch(env.FOLO_DATA_API, {
          method: "POST",
          headers,
          body: JSON.stringify(body)
        ...
        if (!response.ok) {
          console.error(`Failed to fetch Aliyun.AI data, page ${i + 1}: ${response.statusText}`);
          break;
        }
        const data = await response.json();
        if (data && data.data && data.data.length > 0) {
          const filteredItems = data.data.filter((entry) => isDateWithinLastDays(entry.entries.publishedAt, filterDays));
          allAliyunItems.push(...filteredItems.map((entry) => ({
            id: entry.entries.id,
            url: entry.entries.url,
            title: entry.entries.title,
            content_html: entry.entries.content,
          ...
          console.log(`No more data for Aliyun.AI, page ${i + 1}.`);
          break;
        }
      } catch (error) {
        console.error(`Error fetching Aliyun.AI data, page ${i + 1}:`, error);
        break;
      }
      await sleep(Math.random() * 5e3);
    }
...
var aliyun_default = AliyunDataSource;

// src/dataSources/huawei.js
var HuaweiDataSource = {
  fetch: /* @__PURE__ */ __name(async (env) => {
    ...
    try {
      ...
      for (const match of itemMatches) {
        ...
        const extractCDATA = /* @__PURE__ */ __name((xml, tag2) => {
          const cdataMatch = xml.match(new RegExp(`<${tag2}[^>]*><!\\[CDATA\\[([\\s\\S]*?)\\]\\]><\\/${tag2}>`, "i"));
          if (cdataMatch) return cdataMatch[1];
          const normalMatch = xml.match(new RegExp(`<${tag2}[^>]*>([\\s\\S]*?)<\\/${tag2}>`, "i"));
          return normalMatch ? normalMatch[1] : "";
        }, "extractCDATA");
        const title = extractCDATA(itemXml, "title").trim();
        const link2 = extractCDATA(itemXml, "link").trim();
        const pubDate = extractCDATA(itemXml, "pubDate").trim();
        const description = extractCDATA(itemXml, "description");
        const guid = extractCDATA(itemXml, "guid") || link2;
        const contentEncoded = extractCDATA(itemXml, "content:encoded");
        const content = contentEncoded || description;
        let author = extractCDATA(itemXml, "author") || extractCDATA(itemXml, "dc:creator") || extractCDATA(itemXml, "creator") || "\u534E\u4E3A";
        let isoDate = "";
        if (pubDate) {
          try {
...
var huawei_default = HuaweiDataSource;

// src/dataSources/ai-yuanshengsuanli.js
var AiyuanshengDataSource = {
  fetch: /* @__PURE__ */ __name(async (env) => {
    ...
    try {
      ...
      for (const match of itemMatches) {
        ...
        const contentEncoded = extractCDATA(itemXml, "content:encoded");
        const content = contentEncoded || description;
        let author = extractCDATA(itemXml, "author") || extractCDATA(itemXml, "dc:creator") || extractCDATA(itemXml, "creator") || "AI\u4E91\u539F\u751F\u667A\u80FD\u7B97\u529B\u67B6\u6784";
        let isoDate = "";
        if (pubDate) {
          try {
            isoDate = new Date(pubDate).toISOString();
...
var ai_yuanshengsuanli_default = AiyuanshengDataSource;

// src/dataSources/telecom-report.js
var TelecomReportDataSource = {
  type: "telecom-report",
  async fetch(env, foloCookie) {
...
var telecom_report_default = TelecomReportDataSource;
...
var dataSources = {
  ...
  AIopsMedia: { name: "AIOPS", sources: [comsoc_default, lightreading_default, aliyun_default, huawei_default, ai_yuanshengsuanli_default] },
  telecomReport: { name: "\u901A\u4FE1\u62A5\u544A", sources: [telecom_report_default] }
...
// .wrangler/tmp/bundle-w7wb3E/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-w7wb3E/middleware-loader.entry.ts
