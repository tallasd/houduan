...
export async function handleGenAIBestBlogs(request, env) {
    let dateStr;
    let selectedItemsParams = [];
    let formData;
    ...
    try {
        ...
        // If no items selected, try to fetch all for the date (optional, but let's stick to explicit selection for now to avoid massive costs/errors)
        if (selectedItemsParams.length === 0) {
             const errorHtml = generateGenAiPageHtml(env, '生成BestBlogs出错', '<p><strong>No items were selected.</strong> Please go back and select items to analyze.</p>', dateStr, true, null);
             return new Response(errorHtml, { status: 400, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
        }

        console.log(`Generating BestBlogs for ${selectedItemsParams.length} selected items from date ${dateStr}`);

        // 1. Fetch Data (Reuse logic from handleGenAIContent)
        const allFetchedData = {};
        const fetchPromises = [];
        for (const sourceType in dataSources) {
            if (Object.hasOwnProperty.call(dataSources, sourceType)) {
        ...
        for (const selection of selectedItemsParams) {
            ...
            if (item) {
                // Format item for LLM
                let itemText = `类型: ${item.type}\n标题: ${item.title}\n来源: ${item.source || '未知'}\n`;
                if (item.description) itemText += `描述: ${item.description}\n`;
                if (item.details && item.details.content_html) itemText += `内容: ${stripHtml(item.details.content_html).substring(0, 2000)}`; // Truncate to avoid token limits
                selectedContentItems.push(itemText);
            }
        }

        if (selectedContentItems.length === 0) {
             const errorHtml = generateGenAiPageHtml(env, '生成BestBlogs出错', '<p>Selected items invalid.</p>', dateStr, true, selectedItemsParams);
             return new Response(errorHtml, { status: 400, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
        }

        // 2. Call LLM
        const systemPrompt = getSystemPromptBestBlogsAnalysis(dateStr);
        const userPrompt = selectedContentItems.join('\n\n---\n\n');
        
        console.log("Call to Chat (BestBlogs): User prompt length:", userPrompt.length);
        ...
        try {
            ...
            aiResponseText = chunks.join('');
            aiResponseText = removeMarkdownCodeBlock(aiResponseText);
        } catch (error) {
            console.error("Error in BestBlogs Chat API:", error);
            const errorHtml = generateGenAiPageHtml(env, '生成BestBlogs出错', `<p>AI Generation Failed: ${escapeHtml(error.message)}</p>`, dateStr, true, selectedItemsParams);
            return new Response(errorHtml, { status: 500, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
        }
        ...
        try {
            ...
            console.error("Error parsing JSON from AI:", error, aiResponseText);
             const errorHtml = generateGenAiPageHtml(env, '生成BestBlogs出错', `<p>Failed to parse AI response as JSON.</p><pre>${escapeHtml(aiResponseText)}</pre>`, dateStr, true, selectedItemsParams);
             return new Response(errorHtml, { status: 500, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
        }

        // 4. Generate HTML
        const finalHtml = generateBestBlogsPageHtml(env, dateStr, aiData);
        return new Response(finalHtml, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });

    } catch (error) {
        console.error("Error in handleGenAIBestBlogs:", error);
         const errorHtml = generateGenAiPageHtml(env, '生成BestBlogs出错', `<p>Unexpected Error: ${escapeHtml(error.message)}</p>`, dateStr || getISODate(), true, selectedItemsParams);
         return new Response(errorHtml, { status: 500, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
    }
}
